...
class GradeTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.data['elev_ft'] = np.array([7048.15, 7015.69, 7157.89, 7004.84, \
                                        6921.27, 6840.03, 6696.7, \
                                        6735.26, 6554.42, 6445.5])
        
        self.data['dist_ft'] = [0, 1336.0892816, 1336.0892816, 1336.0892816, \
                                  1336.0892816, 1336.12209, 1336.12209, \
                                  1336.12209, 1336.12209, 1336.1548984]
        
        self.data['grade_dec'] = [ 0., -0.0243, 0.1064, -0.1146, -0.0625, \
                          -0.0608, -0.1073, 0.0289, -0.1353, -0.0815]
        
        
        
    def test_haversine_no_bearing(self):
        dist = grade.haversine(self.lat1, self.lon1,
                               self.lat2, self.lon2,
                               get_bearing=False)
    ...
    def test_haversine_with_bearing(self):
        dist, bearing = grade.haversine(self.lat1, self.lon1,
                               self.lat2, self.lon2,
                               get_bearing=True)
    ...
    def test_get_grade_from_dist(self):
        dist_arr, grade_arr = grade.get_grade(self.data.elev_ft, distances = np.array(self.data.dist_ft[1:]))
