...
def bridge_segmentation(bridge_raw):
    ...
    for i in range(len(bridge_raw) - 1):
        if (bridge_raw['id'].iloc[i + 1] -
                bridge_raw['id'].iloc[i] == 1):  # we have consecutive points
            if counter == 0:
                idx.append([])
                # print('row appended')
                c += 1
...
def edge_trim(bridge_ext, data, general_filter):
    ...
    for i in range(len(bridge_ext)):
        ...
        if general_filter:
            data.loc[final_idx[i][0]:final_idx[i][-1],
                     'grade_dec_filtered'] = 0
        else:
            data.loc[final_idx[i][0]:final_idx[i][-1],
                     'grade_dec_unfiltered'] = 0
    return data
